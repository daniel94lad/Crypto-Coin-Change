    ...
    <template v-if="!isLoading">
      <div class="flex flex-col sm:flex-row justify-around items-center">
        ...
        <div class="my-10 sm:mt-0 flex flex-col justify-center text-center">
          ...
          <span class="text-xl">
            {{ convertResult }} {{ fromUSD ? asset.symbol : "$" }}
          </span>
        </div>
      </div>
      <line-chart
      ...
      <table>
        <tr v-for="market in markets" :key="`${market.exchangeId}-${market.priceUsd}`" class="border-b">
          ...
          <td>
            <px-button :is-loading="market.isLoading || false" v-if="!market.url" @click="getWebsite(market)">
...
export default {
  ...
  methods: {
    ...
    getWebsite(exchange) {
      ...
      return api.getExchange(exchange.exchangeId).then(response => {
